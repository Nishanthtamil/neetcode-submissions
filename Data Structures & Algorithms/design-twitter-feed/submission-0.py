class Twitter:
    ...
    def __init__(self):
        self.time=0
        self.tweeMap=defaultdict(list)
        self.followMap=defaultdict(set)

    def postTweet(self, userId: int, tweetId: int) -> None:
        self.tweeMap[userId].append([self.time,tweetId])
        if len(self.tweeMap[userId])>10:
            self.tweeMap[userId].pop(0)
        self.time-=1
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        if len(self.followMap[userId])>=10:
            ...
            for followeeId in self.followMap[userId]:
                if followeeId in self.tweeMap:
                    index=len(self.tweeMap[followeeId])-1
                    time,tweetId=self.tweeMap[followeeId][index]
